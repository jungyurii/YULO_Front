...
const Comment = ({ author, avatar, date, text, children, commentId, boardId, setComments }) => {
  const userId = localStorage.getItem("userId");

  const handleCommentDelete = () => {
    console.log("commentId : ", commentId);

    axios.post("http://127.0.0.1:8080/board/commentDelete", {
      userId: userId,
      boardId: boardId,
      commentId: commentId
    })
      .then(response => {
        ...
        axios.post("http://127.0.0.1:8080/board/detail", {
          ...
          .then(response => {
            console.log("Comments : ", response.data.result.data.comments)
  ...
  return (
    <Box display="flex" mb={2} alignItems="flex-start">
      ...
      <Box>
        <Button variant="text" color="error" startIcon={<Icon>delete</Icon>} onClick={handleCommentDelete}>
          DELETE
        </Button>
